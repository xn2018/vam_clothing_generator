    ...
    for poly in mesh.polygons:
        ...
        for loop_index in poly.loop_indices:
            ...
            if key in uv_map:
                ...
                #
                # First UV vertex of this base vertex
                #
...
def calculate_vertex_normals(
    ...
    """
    Calculate normalized vertex normals from triangle surface normals.
    Surface normals are accumulated to every vertex and normalized.
    IMPORTANT:
        The returned normals are always unit vectors.
    """
    normals = [
        Vector((0.0, 0.0, 0.0))
    ...
    for i in range(vertex_count):
        length_squared = normals[i].length_squared
        if length_squared > EPS:
            normals[i].normalize()
        else:
            #
            # IMPORTANT:
            # Do NOT copy normals[i-1].
            #
            # That would introduce unrelated normal data
            # from another vertex.
            #
            normals[i] = fallback.copy()
    return normals
def recalculate_normals(
    ...
    for i in range(min(10, len(vertex_normals))):
        print(
            i,
            vertex_normals[i],
            "length=",
