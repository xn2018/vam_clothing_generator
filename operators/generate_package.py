...
class VAM_OT_GeneratePackage(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context)-> set[str]:  # type: ignore
        ...
        if clothing_obj is None:
            self.report({'ERROR'}, "Clothing mesh not selected")
            return {'CANCELLED'}
        ...
        dazmesh = build_daz_mesh(obj=clothing_obj, is_body=False, split_material=True)
        wrap_vertices = build_wrap_vertices(dazmesh.mesh)

        wrap_data = calculate_skinwrap_matrix(
            genesis_obj=genesis,
            clothing_mesh=dazmesh.mesh,
...
# 2. Define the second button's operator
class VAM_OT_SPLIT(bpy.types.Operator):
    bl_idname = "vam.ot_split"
    bl_label = "Split Mesh"
